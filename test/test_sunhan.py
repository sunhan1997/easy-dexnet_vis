...
import cv2
ROOT_PATH = r'/home/sunh/6D_grasp/easy-dexnet'
...
TEST_OBJ_FILE = os.path.join(ROOT_PATH, r'data/Gear/Gear.obj')    ############  should be change
...
HDF5_FILE = r'/home/sunh/6D_grasp/easy-dexnet/template/Gear.hdf5'  ############  should be change
OBJ_GROUP = '/home/sunh/6D_grasp/easy-dexnet/data/test/'
...
def display(mesh, grasps, quality_s=None):
    ...
    for g,q in zip(grasps, quality_s):
        ...
        c = c.astype(int)
        rotation = scene.add_grasp(g, color=c)
        center = scene.add_grasp_center(g)

        trasform = np.eye(4)
        trasform[:3, :3] = rotation
        trasform[:3, 3] = center
        grasp_pose.append(trasform)
        grasp_q.append(q)
    pyrender.Viewer(scene, use_raymond_lighting=True)
    np.save('/home/sunh/6D_grasp/easy-dexnet/data/grasp_reault_gear/grasp.npy',grasp_pose)  ############  should be change
    np.save('/home/sunh/6D_grasp/easy-dexnet/data/grasp_reault_gear/grasp_q.npy',grasp_q)   ############  should be change
...
def test_dataset():
    ...
    ff = h5py.File(r'/home/sunh/6D_grasp/easy-dexnet/template/Gear.hdf5', 'a')  ############  should be change
...
def test_to_hdf5():
    ...
    f = h5py.File(r'/home/sunh/6D_grasp/easy-dexnet/template/Gear.hdf5', 'a') ############  should be change
    dex_obj = dex.DexObject.from_file(TEST_OBJ_FILE, config)
